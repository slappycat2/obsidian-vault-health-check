...
def tags_tot(prop_dict):
    tot = 0
    if 'tags' in prop_dict:
        tot = len(prop_dict['tags'].values())

    return tot

# color name numbers are loosely based on:
#    first digit=xl accent color, 2nd dig=%
# The xl fill-color-accent-table is whack!, btw...
# When picking colors, also try turning on High-Contrast for
# some more "Standard" options.
# And be aware that they all change completely when themes
# are applied, so you might want to set the theme, first,
# or better yet, rebuild this structure to include  some themes!

# totals formulas (See, https://openpyxl.readthedocs.io/en/stable/simple_formulae.html)
# from openpyxl.utils import FORMULAE
# "COUNTA" in FORMULAE
# True   # So are: "SUBTOTAL" And "SUM"
# "UNIQUE", "FILTER", "AGGREGATE" will all return False (and require prefix '_xlfn.')
# Below, is what these evaluate to:
#  '=COUNTA(_xlfn.UNIQUE(_xlfn.FILTER(tbl_pros[Property],tbl_pros[IsVisible])))']
#  '=COUNTA(_xlfn.UNIQUE(_xlfn.FILTER(tbl_pros[Values],tbl_pros[IsVisible])))']
#  '=_xlfn.AGGREGATE(3,3,tbl_pros[Property])']
#  '=_xlfn.AGGREGATE(3,3,tbl_pros[Row])']
#  '=_xlfn.AGGREGATE(9,3,tbl_pros[Times Used])']
#  '=IFERROR(IF(_xlfn.AGGREGATE(3,3,tbl_pros[Values])<>SUM(tbl_pros[IsVisible]),"Properties w/Empty Values Detected!",""),"")']
#  '=IFERROR(IF(COUNTA(tbl_pros[Row])<>SUM(tbl_pros[IsVisible]),"Column filters applied--Totals now reflect column filters!",""),"")']
#  '=SUBTOTAL(3,@[Row])']
...
class TabDefinition:
    def __init__(self, tab_id):
        ...
        self.Colors = Colors()
        self.cfg = Config()
        # self.Colors.clr_pur44 = '00CC99FF'
        # self.Colors.clr_pur45 = '00660066'
        # self.Colors.clr_ora64 = '00FFCC99'
        # self.Colors.clr_blud4 = '004F81BD'
        # self.Colors.clr_red20 = '00C0504D'
        # self.Colors.clr_grn30 = '009BBB59'
        # self.Colors.clr_aqu56 = '0033CCCC'
        # self.Colors.clr_gry35 = '00C0C0C0'
        # self.Colors.clr_wht15 = '00F2F2F2'
        # self.Colors.clr_wht00 = '00FFFFFF'
        # self.Colors.clr_blk00 = '00000000'
        self.tab_clr_ftxt_lst = self.Colors.Code_LOV[self.Colors.clr_blud4] # Fill and text colors for grid tab headings
        ...
        self.f_ctot_dup_fnames = f'=COUNTA({self.tbl_name}[{self.hdr_files_col[self.tab_id]}])'
        ...
        self.tab_table_files = {}   # {'tab_cd_table_hdr': {"Row": [4, 8, "", "", False, 'left']}}
        self.tab_def = {
                      'tab_id': self.tab_id
                    , 'showGridLines': True
                    , 'clr_pur44': self.Colors.clr_pur44
                    , 'clr_pur45': self.Colors.clr_pur45
                    , 'clr_ora64': self.Colors.clr_ora64
                    , 'clr_blud4': self.Colors.clr_blud4
                    , 'clr_red20': self.Colors.clr_red20
                    , 'clr_grn30': self.Colors.clr_grn30
                    , 'clr_aqu56': self.Colors.clr_aqu56
                    , 'clr_gry35': self.Colors.clr_gry35
                    , 'clr_wht15': self.Colors.clr_wht15
                    , 'clr_wht00': self.Colors.clr_wht00
                    , 'clr_blk00': self.Colors.clr_blk00
                    ...
                    , 'bdr_thick':          self.bdr_thick
                    , 'bdr_thin':           self.bdr_thin
        ...
        self.tab_def = self.get_def(tab_id)

    def __getitem__(self, key):
        return self.tab_def[key]

    def get_def(self, tab_id):
        self.tab_id = tab_id

        if self.tab_id == 'pros':
            self.tab_def = self.get_pros()
        elif self.tab_id == 'tags':
            self.tab_def = self.get_tags()
        elif self.tab_id == 'summ':
            self.tab_def = self.get_summ()
        elif self.tab_id == 'dups':
            self.tab_def = self.get_dups()
    ...
    def get_pros(self):
        self.tab_id = 'pros'
        clrtb, clrhi = self.cfg.tab_clrs[self.tab_id]
        ...
        self.tab_def['tab_clr_ftxt_lst'] = [clrfl, clrtx]        # Fill and text colors for grid tab headings
        ...
        self.tab_def['tab_cd_table_hdr'] = {
            # [col,row,font,sz, w,t_clr,fill_clr,Bold,Ital,  Align,  val ] = 11
             "Row":  [ 4, 10, '', sz,  8, "", "", True, False, 'center', self.hdr_RowId]
        ...
        self.tab_def['tab_cd_table_dtl'] =  {
            # [col,row,font,sz, w,t_clr,fill_clr,Bold,Ital,  Align,  val ] = 11
             "Row":  [ 4,  0, '', sz,  8, "", "", False, False, 'center', '']
        ...
        # Add calculated fields (tbl_beg_col, tbl_fix_cols, tbl_end_col)
    ...
    def get_tags(self):
        ...
        self.tab_def['tab_table_link_spcrs'] = True  # Always, TRUE for now
        self.tab_def['tab_tots_isVisible_col'] = int(self.tab_def['tbl_end_col'])    #  36
    ...
    def get_summ(self):
        ...
        _, clr_p_hi = self.cfg.tab_clrs['pros']  # ['prop']['tab_clr_ftxt_lst']
        _, clr_t_hi = self.cfg.tab_clrs['tags' ]
        _, clr_d_hi = self.cfg.tab_clrs['dups' ]
        _, clr_s_hi = self.cfg.tab_clrs['summ' ]

        clr_p_fl, clr_p_tx, _ = self.Colors.Code_LOV[clr_p_hi]  # ['prop']['tab_clr_ftxt_lst']
        ...
        self.tab_def['tab_cd_table_hdr'] = {
                              # [col,row,font,sz, w,t_clr,fill_clr,Bold,Ital,  Align,  val ] = 11
              "Row":            [10, 10, '', sz, 12, "", "", True, False, 'center', self.hdr_RowId]
        ...
        self.tab_def['tab_cd_table_dtl'] = {  # [col,row,font,sz, w,t_clr,fill_clr,Bold,Ital,  Align,  val ] = 11
              "Row":            [10, 10, '', sz,  0, "", "", False, False, 'center', self.hdr_RowId]
        ...
        self.tab_def['tab_cd_fixed_grid'] = {  # [col,row,font,sz, w,t_clr,fill_clr,Bold,Ital,  Align,  val ] = 11
            ...
            , 'x_d_file_ctot':   [ 4, 18, '', 11,  0, "", "",             False, False, 'right', "=COUNTA(tbl_dups[Dup Count])"]
    ...
    def get_dups(self):
        ...
        self.tab_def['hdr_links_pfx'] = "Full Pathname"
        ...
        self.tab_def['tab_cd_table_hdr'] = {  # [col,row,font,sz, w,t_clr,fill_clr,Bold,Ital,  Align,  val ] = 11
             'Row':         [4, 10, '', sz, 8, "", clrhi, True, False, 'center', self.hdr_RowId]
            , "Filename":   [5, 10, '', sz, 35, "", clrhi, True, False, 'left', self.hdr_files_col[self.tab_id]]
            , "Dups Found": [6, 10, '', sz, 8, "", clrhi, True, False, 'left', self.hdr_dups_found]
        }
        self.tab_def['tab_cd_table_dtl'] = {  # [col,row,font,sz, w,t_clr,fill_clr,Bold,Ital,  Align,  val ] = 11
             "Row":         [4, 0, '', sz, 0, "", "", False, False, 'center', '']
            , "Filename":   [5, 0, '', sz, 0, "", "", True, False, 'left', '']
            , "Dups Found": [6, 0, '', sz, 0, "", "", False, False, 'center', '']
        }
...
if __name__ == '__main__':
    tab_seq = ['pros']  # , 'xkey_dup_files', 'files'

    for tab_id in tab_seq:
        print(f"\n\nTab ID: {tab_id}")
        this_tab = TabDefinition(tab_id=tab_id)
        tab_def  = this_tab.tab_def
        print(this_tab)
