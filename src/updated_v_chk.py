...
from splash_screen import SplashScreen, ProgressReporter


# This s/b WbDataDef and v_chk should just instantiate the system.
class VaultHealthCheck:   # WbConfig
    def __init__(self, dbug_lvl, progress_reporter=None):
        # super().__init__()
        ...
        if self.progress_reporter:
            self.progress_reporter.update("Loading configuration data...", 5)
        ...
        if self.progress_reporter:
            self.progress_reporter.update("Initializing workbook data...", 5)
        ...
        if self.progress_reporter:
            self.progress_reporter.update("Loading plugin library...", 5)
        plugin_lib = PluginMan(self.cfg['dir_vault'])
        self.obs_plugs = plugin_lib.get_obs_plugs()

        if self.progress_reporter:
            self.progress_reporter.update("Starting vault processing...", 5)
        self.process_vault()
    ...
    def process_vault(self):
        ...
        if self.progress_reporter:
            self.progress_reporter.update(f"Scanning vault: {self.cfg['vault_id']}", 5)

        v_path_obj = Path(self.cfg['dir_vault'])
        
        # Get total file count for progress tracking
        total_files = len(list(v_path_obj.rglob("*.md")))
        processed_files = 0
        
        if self.progress_reporter:
            self.progress_reporter.update(f"Found {total_files} markdown files to process", 5)
        
        for md_file in v_path_obj.rglob("*.md"):
            self.ctot[0] += 1
            processed_files += 1

            # Update progress every 10 files or for large vaults
            if processed_files % max(1, total_files // 20) == 0 or processed_files <= 10:
                if self.progress_reporter:
                    progress = 20 + (processed_files / total_files) * 60  # 20-80% range for file processing
                    self.progress_reporter.set_progress(f"Processing file {processed_files}/{total_files}: {md_file.name}", progress)
        ...
        if self.progress_reporter:
            self.progress_reporter.update("Finalizing vault data...", 10)
        ...
        if self.progress_reporter:
            self.progress_reporter.update("Writing batch data...", 5)
...
def run_vault_health_check_with_splash(dbug_lvl):
    """Run the vault health check with splash screen"""
    splash = SplashScreen("Obsidian Vault Health Check", "v1.0")
    progress_reporter = ProgressReporter(splash, total_steps=100)
    
    def run_health_check():
        # Initialize and run the health check
        vc_def = VaultHealthCheck(dbug_lvl, progress_reporter)
        
        # Create workbook tabs
        progress_reporter.update("Creating workbook tabs...", 5)
        tabs = NewWb(dbug_lvl)
        
        # Export to Excel
        progress_reporter.update("Exporting to Excel...", 5)
        exporter = ExcelExporter(dbug_lvl)
        exporter.export(dbug_lvl)
        
        # Complete
        progress_reporter.complete("Opening workbook...")
        
        return vc_def, tabs, exporter
    
    # Show splash and run the health check
    try:
        result = splash.show_and_run(run_health_check)
        return result
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # Check if we should run with GUI (splash screen)
    use_splash = True
    ...
    if use_splash:
        try:
            ...
            vc_def, tabs, exporter = run_vault_health_check_with_splash(DBUG_LVL)
            print("Vault Health Check completed successfully with splash screen.")
    ...
    if not use_splash:
        # Run without splash screen (original behavior)
        vc_def = VaultHealthCheck(DBUG_LVL)
        tabs = NewWb(DBUG_LVL)
        exporter = ExcelExporter(DBUG_LVL)
        exporter.export(DBUG_LVL)

    # Optional debug output (only when DBUG_LVL > 90)
    if DBUG_LVL > 90:
