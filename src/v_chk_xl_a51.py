...
class ExcelExporter:
    def __init__(self):
        ...
        self.tab_seq = ['summ', 'pros', 'tags', 'dups', 'xyml']  # ['summ', 'pros', 'tags', 'dups']
        self.tabs_built = {}
        self.wb_tabs_done = {}
        self.ctrl_tots = {
             'pros': 0
    ...
    # =================================================================================
        ...
        # self.export_summary(wb, self.wb_tabs_done['summ'], self.tabs_built['summ'])
        if self.DBUG_AREA51:
            self.export_area51(wb)
    ...
    def export_tab(self, wb):
        ...
        for prop_name, values_dict in sorted(self.wb_def['wb_data']['obs_props'].items()):
            ...
            if tab_id == 'dups' and prop_name != 'xkey_dups':
                    continue
            if tab_id == 'xyml' and prop_name != 'xkey_xyml':
                    continue
            if tab_id == 'pros' and prop_name in ["tags", "tags:"]:
                continue
            ...
            for value_item, value_files_list in sorted_values_dict:
                ...
                for dbug_key, dtl_col_def_list in this_row_dtl.items():
    ...
    def obs_hyperlink(self, file):
        file_link = f"{urllib.parse.quote(obs_file, safe=':/')}"
        obs_link_text = obs_file.replace(".md", "")
        obs_link = f'=hyperlink("obsidian://open?vault={self.vault_id}&file={file_link}","{obs_link_text}")'

        return obs_link, obs_link_text
    ...
    def export_dups(self, wb):
        ...
        highest_dup_count = 0
        for file, dir_list in sorted(self.wb_def['wb_data']['obs_props']['xkey_dups'].items()):
            if len(dir_list) == 1:
                continue
    ...
    def xl_set_border(self, ws, cell_range=None, border_type="thin", color="000000"):
    ...
    def export_area51(self, wb):
        ...
        for tab_id in self.tab_seq:
            ...
            for col, cell_def in a51_def['tab_cd_table_hdr'].items():
                cell_def[0] = col_idx
                _, _ = self.export_cell(area51_tab, cell_def, '', row_idx)
            ...
            a51_tab_def = self.wb_def['wb_tabs'][tab_id]
            sub_defs = {}
            a52_sub_defs = {}
            ...
            tab_tbl_style = self.wb_def['wb_tabs'][tab_id]['tab_table_style']
            ...
            for col, cell_def in a52_def['tab_cd_table_hdr'].items():
                ...
                if col_idx == a52_tab_col_idx:
                    cell_def[10] = tab_id
                _, _ = self.export_cell(area52_tab, cell_def, '', row_idx)
                col_idx += 1
...
#       cell = summary_tab.cell(row=4, column=5, value=f"{self.c_date}")
        ...
        cell = summary_tab.cell(row=5, column=5, value=f"{self.cfg_pname}")
        cell.font = Font(size=11, bold=True)
        cell = summary_tab.cell(row=6, column=5, value=f"{self.cfg_num:04d}")
        cell.font = Font(size=11, bold=True)
#       self.xl_set_border(summary_tab, "C4:H6", "thin", tab_color)
...
#       # get totals
        ...
        self.tags_tab_list = []
        len_pros = len(self.wb_def['wb_data']['obs_props'])
        # len_tags = len(cfg.pros["tags"])
        len_tags = 0
        tot_pros = 0
        tot_prop_vals = 0
        tot_tags = 0
        # print(f"cfg_pros: {self.cfg.pros}")
        for prop_name, values_dict in self.wb_def['wb_data']['obs_props'].items():
            if prop_name != "tags":
                tot_pros += 1
            for value_item, value_files_list in values_dict.items():
